...
"""
  ...
  e.g. STORE_DIRECTORY=/data/ ./download-metrics.py
...
def download_monthly_metrics(trading_type, symbols, num_symbols, years, months, start_date, end_date, folder, checksum):
  current = 0
  date_range = None
    ...
    for year in years:
      for month in months:
        ...
        if current_date >= start_date and current_date <= end_date:
          ...
          if checksum == 1:
            checksum_path = get_path(trading_type, "metrics", "monthly", symbol)
            checksum_file_name = "{}-metrics-{}-{}.zip.CHECKSUM".format(symbol.upper(), year, '{:02d}'.format(month))
...
if __name__ == "__main__":
    ...
    if args.dates:
      ...
      dates = [date.strftime("%Y-%m-%d") for date in dates]
        ...
        #download_monthly_metrics(args.type, symbols, num_symbols, args.years, args.months, args.startDate, args.endDate, args.folder, args.checksum)
